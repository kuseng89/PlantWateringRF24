...
void SetSensorLimit(unsigned char lim);
...
void ShowSensorInfo();
void BatteryState(unsigned char i);
void TankLevel(unsigned char i);
///////////////////////////////////////
...
void ShowSensorList() {
  ...
  if (VIEW==1) {
    arrowPos=0;
    activeListEntry=0;
    for (char i = 0; i < NR_OF_SENSORS; i++) {
...
void CleanSensorList() {
  for (char i = 0; i < NR_OF_SENSORS; i++) {
    if ((millis() - int(sensorList[i].ms)) > SENSOR_TIMEOUT && int(sensorList[i].ms) > 0) {
  ...
  for (char i = 0; i < NR_OF_SENSORS - 1; i++) {
